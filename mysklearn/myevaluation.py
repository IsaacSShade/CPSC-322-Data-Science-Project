...
# 11/12/25
# 
# Description: Functions for splitting datasets into training and testing sets
##############################################
...
def train_test_split(X, y, test_size=0.33, random_state=None, shuffle=True):
    ...
    if isinstance(test_size, float):
        num_test = int(np.ceil(test_size * n_samples))
    
    num_train = n_samples - num_test
...
def kfold_split(X, n_splits=5, random_state=None, shuffle=False):
    ...
    indices = np.arange(n_samples)
    fold_length = n_samples // n_splits
    remainder = n_samples % n_splits
    sizes = [fold_length + 1 if k < remainder else fold_length for k in range(n_splits)]
    ...
    for _, size in enumerate(sizes):
        ...
        test_indexes = indices[start:stop].tolist()
        train_indexes = np.concatenate([indices[:start], indices[stop:]]).tolist()
...
def bootstrap_sample(X, y=None, n_samples=None, random_state=None):
    ...
    for i in range(n_samples):
        sample_index = rng.choice(indices)
...
def confusion_matrix(y_true, y_pred, labels):
    ...
    for i, prediction in enumerate(y_pred):
        true_label = y_true[i]
        matrix[labels.index(true_label)][labels.index(prediction)] += 1
...
def binary_precision_score(y_true, y_pred, labels=None, pos_label=None):
    ...
    for true_value, predicted_value in zip(y_true, y_pred):
        if predicted_value == pos_label:
            if true_value == pos_label:
                true_positives += 1
            else:
                false_positives += 1
